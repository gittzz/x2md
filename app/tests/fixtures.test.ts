...
test("golden fixtures: 博主 tweets/articles 批量抓取写入预期文件", () => {
  ...
  const tweets = handleProfileCaptureSave(fixture("profile_tweets.json"), profileCfg, appDir);
  assert.equal(tweets.saved.length, 1);
  assert.match(readFileSync(tweets.saved[0], "utf8"), /profile tweet one/);

  const articles = handleProfileCaptureSave(fixture("profile_articles.json"), profileCfg, appDir);
